<template>
  <div class=" flex flex-row justify-between items-start  gap-4 w-full h-full ">
    <input class="flex-1 rounded-2xl p-4 outline-primary-600 h-full " placeholder="Add a review" type="text">
    <div class="flex flex-col justify-start items-center gap-4">
      <div class=" flex flex-row justify-between items-center w-full gap-4 ">

        <div class=" rotate-90 bg-gray-100 p-2 rounded-full cursor-pointer ">
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
              d="M14.475 12L7.12499 4.65C6.87499 4.4 6.75415 4.10417 6.76249 3.7625C6.77082 3.42083 6.89999 3.125 7.14999 2.875C7.39999 2.625 7.69582 2.5 8.03749 2.5C8.37915 2.5 8.67499 2.625 8.92499 2.875L16.6 10.575C16.8 10.775 16.95 11 17.05 11.25C17.15 11.5 17.2 11.75 17.2 12C17.2 12.25 17.15 12.5 17.05 12.75C16.95 13 16.8 13.225 16.6 13.425L8.89999 21.125C8.64999 21.375 8.35832 21.4958 8.02499 21.4875C7.69165 21.4792 7.39999 21.35 7.14999 21.1C6.89999 20.85 6.77499 20.5542 6.77499 20.2125C6.77499 19.8708 6.89999 19.575 7.14999 19.325L14.475 12Z"
              fill="#228d73" />
          </svg>

        </div>

        <span class=" font-semibold text-blackich-200 text-2xl ">4.5</span>

        <div class=" -rotate-90 bg-gray-100 p-2 rounded-full cursor-pointer ">
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
              d="M14.475 12L7.12499 4.65C6.87499 4.4 6.75415 4.10417 6.76249 3.7625C6.77082 3.42083 6.89999 3.125 7.14999 2.875C7.39999 2.625 7.69582 2.5 8.03749 2.5C8.37915 2.5 8.67499 2.625 8.92499 2.875L16.6 10.575C16.8 10.775 16.95 11 17.05 11.25C17.15 11.5 17.2 11.75 17.2 12C17.2 12.25 17.15 12.5 17.05 12.75C16.95 13 16.8 13.225 16.6 13.425L8.89999 21.125C8.64999 21.375 8.35832 21.4958 8.02499 21.4875C7.69165 21.4792 7.39999 21.35 7.14999 21.1C6.89999 20.85 6.77499 20.5542 6.77499 20.2125C6.77499 19.8708 6.89999 19.575 7.14999 19.325L14.475 12Z"
              fill="#228d73" />
          </svg>

        </div>



      </div>
      <button class="  bg-primary-600 w-full py-2 text-xl rounded-lg text-white font-semibold">
        Post
      </button>
    </div>
  </div>
</template>

<script setup>

</script>



<style></style>
